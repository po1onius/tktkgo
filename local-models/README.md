...
`uv sync` 会在当前目录生成独立的 `.venv`。也可以在同步完成后使用 `.venv/bin/python provider.py` 启动。缺失权重会在服务启动时下载到本目录的 `models/`。默认使用 WhisperX 官方中文对齐模型 `jonatasgrosman/wav2vec2-large-xlsr-53-chinese-zh-cn` 和 CPU 推理；该模型卡使用 Apache-2.0 许可证。服务在模型完成加载后才通过健康检查。

该服务不是语音转写服务：请求必须同时提供 WAV 和确定口播原文，输出文本只能来自原文，CTC 模型只计算字符时间戳。默认 uv 配置固定使用 PyTorch CPU 索引；GPU 部署需要按 WhisperX 官方 CUDA 要求手动修改依赖索引和 `device`，不要在同一环境混装 CPU 与 CUDA 运行时。
...
本地服务默认分别监听 `http://127.0.0.1:8102` 和 `http://127.0.0.1:8101`。在 `provider/providers.toml` 中配置相同 `base_url`，并将对应模型加入 `models.alignment.whisperx` 或 `models.speech.cosy_voice`。网关会探测健康接口，只有部署模型与目录模型一致时才向前端标记为可用。
